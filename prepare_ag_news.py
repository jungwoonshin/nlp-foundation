...
def prepare_ag_news(data_dir: Path) -> Path:
    """Fetch train.csv into data/raw/ and write one article per line to ag_news.txt."""
    ...
    if out_path.is_file() and out_path.stat().st_mtime >= raw_csv.stat().st_mtime:
        return out_path
    ...
    with raw_csv.open(encoding="utf-8", newline="") as src, out_path.open(
        ...
        for row in csv.reader(src):
            ...
            n_docs += 1
    print(f"wrote {n_docs:,} articles -> {out_path}")
    return out_path
